...
``python client_starter.py {имя сервера} {порт} -n или --name {имя пользователя} -p или -password {пароль}``
...
* ``python client_starter.py ip_address some_port``
...
* ``python client_starter.py ip_address some_port -n test1 -p 123``
